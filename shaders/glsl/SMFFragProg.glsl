...
vec4 GetSplatDiffuse(vec4 diffuseCol, vec2 uv, out vec2 splatDetailStrength) {
	...
	float remaining = 1.0 - splatCofac.r;
	vec4 splatDiffuse = texture2D(splatDetailNormalTex1, splatTexCoord0.pq * 1.0) * splatCofac.r;
	if (remaining > 0.0) {
		float maxTake = min(splatCofac.g, remaining);
		splatDiffuse += texture2D(splatDetailNormalTex2, splatTexCoord0.st * 1.0) * maxTake;
		remaining -= maxTake;
	}
